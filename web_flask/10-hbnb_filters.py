...
task 11: return list of states, based on task 6 code
...
def get_state_by_id(id):
    """ task 9
    returns a dictionary of objects, which
    will be used to get state by id
    also show cities per state on
    jinja2, while using sort on jinja2
    """
    states = storage.all(State)
    if (id is None):
...
def hbnb_filters_func():
    """ handle requests on hbnb_filters """
    states = storage.all(State)
